...
def get_filters():
	...
	while city.lower() not in ("chicago", "new york", "washington"):
		...
		print("incorrect answer!! please type one of the town's name given") 
		city = input("which data's town do you want to analyze chicago, new york or washington? type one of those towns :\n ")
		
	CITY_DATA_COMPARE.pop(city.lower())
	...
	while month.lower() not in ("january", "february", "march", "april", "may", "june", "all"):
		...
		print("incorrect answer!! please type one of the months given or 'all' to analyze them all") 
		month = input("which month do you want to analyze: January, February, March, April, May, June or all? type 'all' or one of those months (please respect the case...):\n ")  

	month_day_compare["month_compare"] = month.lower() 
	...
	while day.lower() not in ("monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday", "all"): 
		print("incorrect answer!! please type one of the day given or type 'all' to analyze them all") 
		day = input("which day do you want to analyze: Monday, Tuesday, Wednesday, Thurday, Friday, Saturday, Sunday or all of them? type 'all' or one of those days (please respect the case...):\n ?:\n  ")  
	
	month_day_compare["day_compare"] = day.lower() 
...
def load_data(city, month, day):
	...
	if month.lower() == "all":
		...
		if day.lower() =="all":  # load data for a specific month with all the days
			df = pd. read_csv(CITY_DATA[city.lower()]) 
			df['Start Time'] = pd.to_datetime(df['Start Time']) 
...
# to display raw data in groups of 5 lines
def raw_data(city):
	raw_quest = input("\n\nWould you see the raw data of {} ?, type 'yes' or 'no' \n".format(city.lower())) 
	while raw_quest.lower() not in ("yes", "no"):
		print("\nType 'yes' or 'no' please, thank you")
		raw_quest = input("\nWould you see the raw data of {} ?, type 'yes' or 'no' \n".format(city.lower()))

	if raw_quest.lower() == "yes":
		raw_df = pd. read_csv(CITY_DATA[city.lower()])
		raw_df = pd.DataFrame(raw_df)
		max_line = len(raw_df)
		j = 5
		print(raw_df.iloc[0:5,:])

		while j < (max_line-5):
			raw_quest_line = input("\n\nWould you see the next 5 lines ? type 'yes' or 'no' \n")
			while raw_quest_line.lower() not in ("yes", "no"):
				print("\nType 'yes' or 'no' please, thank you")
				raw_quest_line = input("\n\nWould you see the next 5 lines ? type 'yes' or 'no' \n")

			if raw_quest_line.lower() == "yes":				
				print(raw_df.iloc[j:j+5, :])
				j = j+5
			else:
				break
...
def time_stats(df):
	...
	time_stats_view = input("\nWould you like to view some time statistics? type 'yes' or 'no' \n")
	while time_stats_view.lower() not in ("yes", "no"):
		print("type 'yes' or 'no' please, thank you")
		time_stats_view = input("Would you like to view some time statistics? type 'yes' or 'no' \n")
	if time_stats_view.lower() == "yes":
...
def station_stats(df):
	...
	while station_stats_view.lower() not in ("yes", "no"):
		print("type 'yes' or 'no' please, thank you")
		station_stats_view = input("Would you like to view some station statistics? type 'yes' or 'no' \n")
	if station_stats_view.lower() == "yes":
...
def trip_duration_stats(df):
	...
	while trip_duration_stats_view.lower() not in ("yes", "no"):
		print("type 'yes' or 'no' please, thank you")
		trip_duration_stats_view = input("Would you like to view some trip duration statistics? type 'yes' or 'no' \n")

	if trip_duration_stats_view.lower() == "yes":
...
def compare_towns():
	...
	while compare_test.lower() not in ("yes", "no"):
		print("type 'yes' or 'no' please, thank you")
		compare_test = input("would you like to compare statistics with another town? type 'yes' or 'no' \n")

	# display towns except the one choosed before
	if compare_test.lower() == "yes":
		...
		while town_compare.lower() not in towns_set[:]:
		...
		CITY_DATA_COMPARE[city_one] = CITY_DATA[city_one]
